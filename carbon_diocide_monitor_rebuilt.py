...
class DatabaseManager:
    def __init__(self):
        db_dir = os.path.join(BASE_DIR, "DB_Logs")
        os.makedirs(db_dir, exist_ok=True)
        self.db_path = os.path.join(db_dir, "co2_monitoring.db")
        self.init_db()

    def get_connection(self):
        # check_same_thread=False를 주어 멀티스레드 환경에서 안전하게 접근
    ...
    def insert_log(self, today_str, time_str, sensor_index, port, record_value):
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO co2_logs (today_str, time_str, sensor_index, port, record_value)
                VALUES (?, ?, ?, ?, ?)
            """, (today_str, time_str, sensor_index, port, str(record_value)))
            conn.commit()

    def export_to_excel(self):
        """프로그램 종료 시 DB 데이터를 날짜별/센서별 엑셀 파일로 일괄 내보내기"""
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT today_str, time_str, sensor_index, port, record_value FROM co2_logs")
        ...
        # 데이터를 날짜별, 센서별로 분류
        # 구조: { (today_str, sensor_index): [row_data, ...] }
        grouped_data = {}
        for row in rows:
            today_str, time_str, sensor_index, port, val = row
            # 숫자로 변환 가능한 것은 숫자로 복원
            try:
                if "." in val:
                    val = float(val)
                else:
                ...
                pass # "Error: ..." 같은 문자열 형태인 경우 그대로 유지

            key = (today_str, sensor_index)
            if key not in grouped_data:
                grouped_data[key] = []
            grouped_data[key].append([today_str, time_str, sensor_index + 1, port, val])

        # 엑셀 파일 생성 (기존 openpyxl 스타일 적용 유지)
        ...
        for (today_str, sensor_index), data_rows in grouped_data.items():
            file_path = os.path.join(save_dir, f"CO2_log_{today_str}_Sensor{sensor_index + 1}.xlsx")
...
class SerialThread(QThread):
    ...
    def __init__(self, port_name, sensor_index, db_manager): # db_manager 인자 추가
        super().__init__()
        self.current_port = port_name  
        self.sensor_index = sensor_index
        self.db_manager = db_manager # 저장
            ...
            try:
                ...
                while ser.is_open and not self.isInterruptionRequested():
                    ...
                    if now_struct.tm_min != last_saved_minute:
                        ...
                        # [변경점] 엑셀 직접 저장 대신 SQLite DB에 안전하게 적재
                        try:
                            self.db_manager.insert_log(today_str, time_str, self.sensor_index, self.current_port, record_value)
                        except Exception as e:
                            self.write_log(f"DB 저장 오류: {str(e)}")
...
class CO2MonitorApp(QMainWindow):
    def __init__(self, target_ports, db_manager): # db_manager 인자 추가
        super().__init__()
    ...
    def start_threads(self):
        for i, port_name in enumerate(self.target_ports):  
            thread = SerialThread(port_name, i, self.db_manager) # DB 매니저 전달
    ...
    def closeEvent(self, event):
        ...
        # [추가] 프로그램이 종료될 때 DB에 쌓인 데이터를 예쁜 엑셀 파일로 일괄 내보내기 수행
        try:
...
if __name__ == "__main__":
    ...
    if setup_dialog.exec_() == QDialog.Accepted:
        ...
        window = CO2MonitorApp(selected_ports, db_manager) # db_manager 전달
        window.show()
        sys.exit(app.exec_())
